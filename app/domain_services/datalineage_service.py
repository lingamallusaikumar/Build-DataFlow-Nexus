...
class DataLineageService:
    """Enterprise service layer for DataLineage with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = DataLineage.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(DataLineage, key):
                    query = query.filter(getattr(DataLineage, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = DataLineage.find_by_id(record_id)
        if not record:
            raise ValueError(f"DataLineage with id {record_id} not found")
        return record
