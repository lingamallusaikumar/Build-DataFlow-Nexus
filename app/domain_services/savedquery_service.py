...
class SavedQueryService:
    """Enterprise service layer for SavedQuery with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = SavedQuery.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(SavedQuery, key):
                    query = query.filter(getattr(SavedQuery, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = SavedQuery.find_by_id(record_id)
        if not record:
            raise ValueError(f"SavedQuery with id {record_id} not found")
        return record
