...
class MetricService:
    """Enterprise service layer for Metric with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = Metric.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(Metric, key):
                    query = query.filter(getattr(Metric, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = Metric.find_by_id(record_id)
        if not record:
            raise ValueError(f"Metric with id {record_id} not found")
        return record
