...
class WebhookDeliveryService:
    """Enterprise service layer for WebhookDelivery with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = WebhookDelivery.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(WebhookDelivery, key):
                    query = query.filter(getattr(WebhookDelivery, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = WebhookDelivery.find_by_id(record_id)
        if not record:
            raise ValueError(f"WebhookDelivery with id {record_id} not found")
        return record
