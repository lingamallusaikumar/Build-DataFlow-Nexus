...
class DataSchemaService:
    """Enterprise service layer for DataSchema with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = DataSchema.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(DataSchema, key):
                    query = query.filter(getattr(DataSchema, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = DataSchema.find_by_id(record_id)
        if not record:
            raise ValueError(f"DataSchema with id {record_id} not found")
        return record
