...
class QuotaService:
    """Enterprise service layer for Quota with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = Quota.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(Quota, key):
                    query = query.filter(getattr(Quota, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = Quota.find_by_id(record_id)
        if not record:
            raise ValueError(f"Quota with id {record_id} not found")
        return record
