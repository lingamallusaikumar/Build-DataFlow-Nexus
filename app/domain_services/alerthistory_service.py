...
class AlertHistoryService:
    """Enterprise service layer for AlertHistory with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = AlertHistory.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(AlertHistory, key):
                    query = query.filter(getattr(AlertHistory, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = AlertHistory.find_by_id(record_id)
        if not record:
            raise ValueError(f"AlertHistory with id {record_id} not found")
        return record
