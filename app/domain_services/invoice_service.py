...
class InvoiceService:
    """Enterprise service layer for Invoice with business logic and validation."""
    ...
    def get_by_id(record_id):
        record = Invoice.find_by_id(record_id)
        if not record:
            raise ValueError(f"Invoice with id {record_id} not found")
        return record
