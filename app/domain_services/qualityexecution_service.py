...
class QualityExecutionService:
    """Enterprise service layer for QualityExecution with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = QualityExecution.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(QualityExecution, key):
                    query = query.filter(getattr(QualityExecution, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = QualityExecution.find_by_id(record_id)
        if not record:
            raise ValueError(f"QualityExecution with id {record_id} not found")
        return record
