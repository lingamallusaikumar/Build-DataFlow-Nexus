...
class AlertService:
    """Enterprise service layer for Alert with business logic and validation."""
    ...
    def get_by_id(record_id):
        record = Alert.find_by_id(record_id)
        if not record:
            raise ValueError(f"Alert with id {record_id} not found")
        return record
