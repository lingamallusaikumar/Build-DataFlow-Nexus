...
class DeadLetterQueueService:
    """Enterprise service layer for DeadLetterQueue with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = DeadLetterQueue.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(DeadLetterQueue, key):
                    query = query.filter(getattr(DeadLetterQueue, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = DeadLetterQueue.find_by_id(record_id)
        if not record:
            raise ValueError(f"DeadLetterQueue with id {record_id} not found")
        return record
