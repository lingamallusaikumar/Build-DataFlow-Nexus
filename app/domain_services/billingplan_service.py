...
class BillingPlanService:
    """Enterprise service layer for BillingPlan with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = BillingPlan.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(BillingPlan, key):
                    query = query.filter(getattr(BillingPlan, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = BillingPlan.find_by_id(record_id)
        if not record:
            raise ValueError(f"BillingPlan with id {record_id} not found")
        return record
