...
class TenantService:
    """Enterprise service layer for Tenant with business logic and validation."""
    ...
    def get_by_id(record_id):
        record = Tenant.find_by_id(record_id)
        if not record:
            raise ValueError(f"Tenant with id {record_id} not found")
        return record
