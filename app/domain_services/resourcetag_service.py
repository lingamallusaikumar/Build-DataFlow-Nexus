...
class ResourceTagService:
    """Enterprise service layer for ResourceTag with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = ResourceTag.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(ResourceTag, key):
                    query = query.filter(getattr(ResourceTag, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = ResourceTag.find_by_id(record_id)
        if not record:
            raise ValueError(f"ResourceTag with id {record_id} not found")
        return record
