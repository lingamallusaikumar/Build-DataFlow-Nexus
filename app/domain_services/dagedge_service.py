...
class DagEdgeService:
    """Enterprise service layer for DagEdge with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = DagEdge.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(DagEdge, key):
                    query = query.filter(getattr(DagEdge, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = DagEdge.find_by_id(record_id)
        if not record:
            raise ValueError(f"DagEdge with id {record_id} not found")
        return record
