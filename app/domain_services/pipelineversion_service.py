...
class PipelineVersionService:
    """Enterprise service layer for PipelineVersion with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = PipelineVersion.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(PipelineVersion, key):
                    query = query.filter(getattr(PipelineVersion, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = PipelineVersion.find_by_id(record_id)
        if not record:
            raise ValueError(f"PipelineVersion with id {record_id} not found")
        return record
