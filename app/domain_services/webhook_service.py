...
class WebhookService:
    """Enterprise service layer for Webhook with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = Webhook.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(Webhook, key):
                    query = query.filter(getattr(Webhook, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = Webhook.find_by_id(record_id)
        if not record:
            raise ValueError(f"Webhook with id {record_id} not found")
        return record
