...
class RolePermissionService:
    """Enterprise service layer for RolePermission with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = RolePermission.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(RolePermission, key):
                    query = query.filter(getattr(RolePermission, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = RolePermission.find_by_id(record_id)
        if not record:
            raise ValueError(f"RolePermission with id {record_id} not found")
        return record
