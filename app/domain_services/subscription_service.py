...
class SubscriptionService:
    """Enterprise service layer for Subscription with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = Subscription.query.filter_by(is_deleted=False)
        if filters:
    ...
    def get_by_id(record_id):
        record = Subscription.find_by_id(record_id)
        if not record:
            raise ValueError(f"Subscription with id {record_id} not found")
        return record
