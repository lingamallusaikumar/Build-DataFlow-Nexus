...
class SessionService:
    """Enterprise service layer for Session with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = Session.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(Session, key):
                    query = query.filter(getattr(Session, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = Session.find_by_id(record_id)
        if not record:
            raise ValueError(f"Session with id {record_id} not found")
        return record
