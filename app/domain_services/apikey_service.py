...
class ApiKeyService:
    """Enterprise service layer for ApiKey with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = ApiKey.query.filter_by(is_deleted=False)
        if filters:
    ...
    def get_by_id(record_id):
        record = ApiKey.find_by_id(record_id)
        if not record:
            raise ValueError(f"ApiKey with id {record_id} not found")
        return record
