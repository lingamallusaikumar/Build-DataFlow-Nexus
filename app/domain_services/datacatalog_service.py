...
class DataCatalogService:
    """Enterprise service layer for DataCatalog with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = DataCatalog.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(DataCatalog, key):
                    query = query.filter(getattr(DataCatalog, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = DataCatalog.find_by_id(record_id)
        if not record:
            raise ValueError(f"DataCatalog with id {record_id} not found")
        return record
