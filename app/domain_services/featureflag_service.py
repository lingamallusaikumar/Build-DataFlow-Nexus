...
class FeatureFlagService:
    """Enterprise service layer for FeatureFlag with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = FeatureFlag.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(FeatureFlag, key):
                    query = query.filter(getattr(FeatureFlag, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = FeatureFlag.find_by_id(record_id)
        if not record:
            raise ValueError(f"FeatureFlag with id {record_id} not found")
        return record
