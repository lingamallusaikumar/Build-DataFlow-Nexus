...
class TaskRunService:
    """Enterprise service layer for TaskRun with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = TaskRun.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(TaskRun, key):
                    query = query.filter(getattr(TaskRun, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = TaskRun.find_by_id(record_id)
        if not record:
            raise ValueError(f"TaskRun with id {record_id} not found")
        return record
