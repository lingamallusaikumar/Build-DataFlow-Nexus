...
class AuditLogService:
    """Enterprise service layer for AuditLog with business logic and validation."""
    ...
    def get_by_id(record_id):
        record = AuditLog.find_by_id(record_id)
        if not record:
            raise ValueError(f"AuditLog with id {record_id} not found")
        return record
