...
class QuotaUsageService:
    """Enterprise service layer for QuotaUsage with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = QuotaUsage.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(QuotaUsage, key):
                    query = query.filter(getattr(QuotaUsage, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = QuotaUsage.find_by_id(record_id)
        if not record:
            raise ValueError(f"QuotaUsage with id {record_id} not found")
        return record
