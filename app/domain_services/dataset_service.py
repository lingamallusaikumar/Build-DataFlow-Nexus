...
class DatasetService:
    """Enterprise service layer for Dataset with business logic and validation."""
        ...
        if filters:
            for key, value in filters.items():
                if hasattr(Dataset, key):
                    query = query.filter(getattr(Dataset, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = Dataset.find_by_id(record_id)
        if not record:
            raise ValueError(f"Dataset with id {record_id} not found")
        return record
