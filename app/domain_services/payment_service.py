...
class PaymentService:
    """Enterprise service layer for Payment with business logic and validation."""
    ...
    def get_by_id(record_id):
        record = Payment.find_by_id(record_id)
        if not record:
            raise ValueError(f"Payment with id {record_id} not found")
        return record
