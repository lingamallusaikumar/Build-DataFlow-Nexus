...
class UserRoleService:
    """Enterprise service layer for UserRole with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = UserRole.query.filter_by(is_deleted=False)
        if filters:
    ...
    def get_by_id(record_id):
        record = UserRole.find_by_id(record_id)
        if not record:
            raise ValueError(f"UserRole with id {record_id} not found")
        return record
