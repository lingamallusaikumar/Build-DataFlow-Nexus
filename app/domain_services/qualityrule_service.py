...
class QualityRuleService:
    """Enterprise service layer for QualityRule with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = QualityRule.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(QualityRule, key):
                    query = query.filter(getattr(QualityRule, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = QualityRule.find_by_id(record_id)
        if not record:
            raise ValueError(f"QualityRule with id {record_id} not found")
        return record
