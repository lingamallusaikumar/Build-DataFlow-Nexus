...
class DagNodeService:
    """Enterprise service layer for DagNode with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = DagNode.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(DagNode, key):
                    query = query.filter(getattr(DagNode, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = DagNode.find_by_id(record_id)
        if not record:
            raise ValueError(f"DagNode with id {record_id} not found")
        return record
