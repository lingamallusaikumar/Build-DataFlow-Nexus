...
class PipelineService:
    """Enterprise service layer for Pipeline with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = Pipeline.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(Pipeline, key):
                    query = query.filter(getattr(Pipeline, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = Pipeline.find_by_id(record_id)
        if not record:
            raise ValueError(f"Pipeline with id {record_id} not found")
        return record
