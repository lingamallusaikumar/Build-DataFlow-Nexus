...
class PipelineRunService:
    """Enterprise service layer for PipelineRun with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = PipelineRun.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(PipelineRun, key):
                    query = query.filter(getattr(PipelineRun, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = PipelineRun.find_by_id(record_id)
        if not record:
            raise ValueError(f"PipelineRun with id {record_id} not found")
        return record
