...
class NotificationConfigService:
    """Enterprise service layer for NotificationConfig with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = NotificationConfig.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(NotificationConfig, key):
                    query = query.filter(getattr(NotificationConfig, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = NotificationConfig.find_by_id(record_id)
        if not record:
            raise ValueError(f"NotificationConfig with id {record_id} not found")
        return record
    ...
    def create(data):
        ...
        record = NotificationConfig(**data)
