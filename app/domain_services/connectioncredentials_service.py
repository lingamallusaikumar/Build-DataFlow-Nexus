...
class ConnectionCredentialsService:
    """Enterprise service layer for ConnectionCredentials with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = ConnectionCredentials.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(ConnectionCredentials, key):
                    query = query.filter(getattr(ConnectionCredentials, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = ConnectionCredentials.find_by_id(record_id)
        if not record:
            raise ValueError(f"ConnectionCredentials with id {record_id} not found")
        return record
    ...
    def create(data):
        ...
        record = ConnectionCredentials(**data)
