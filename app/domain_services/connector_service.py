...
class ConnectorService:
    """Enterprise service layer for Connector with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = Connector.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(Connector, key):
                    query = query.filter(getattr(Connector, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = Connector.find_by_id(record_id)
        if not record:
            raise ValueError(f"Connector with id {record_id} not found")
        return record
