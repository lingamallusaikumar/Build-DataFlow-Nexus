...
class RoleService:
    """Enterprise service layer for Role with business logic and validation."""
    ...
    def get_by_id(record_id):
        record = Role.find_by_id(record_id)
        if not record:
            raise ValueError(f"Role with id {record_id} not found")
        return record
