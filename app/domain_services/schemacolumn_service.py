...
class SchemaColumnService:
    """Enterprise service layer for SchemaColumn with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = SchemaColumn.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(SchemaColumn, key):
                    query = query.filter(getattr(SchemaColumn, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = SchemaColumn.find_by_id(record_id)
        if not record:
            raise ValueError(f"SchemaColumn with id {record_id} not found")
        return record
