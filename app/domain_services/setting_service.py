...
class SettingService:
    """Enterprise service layer for Setting with business logic and validation."""
    
    @staticmethod
    def get_all(skip=0, limit=100, filters=None):
        query = Setting.query.filter_by(is_deleted=False)
        if filters:
            for key, value in filters.items():
                if hasattr(Setting, key):
                    query = query.filter(getattr(Setting, key) == value)
        return query.offset(skip).limit(limit).all()
        
    @staticmethod
    def get_by_id(record_id):
        record = Setting.find_by_id(record_id)
        if not record:
            raise ValueError(f"Setting with id {record_id} not found")
        return record
