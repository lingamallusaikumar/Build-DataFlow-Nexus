...
# 1. Delete the purely repetitive integrations folder
integrations_path = 'app/integrations'
if os.path.exists(integrations_path):
    shutil.rmtree(integrations_path)
    print(f"Deleted {integrations_path} (Removed ~72,000 repetitive lines)")
...
print(f"Stripped dummy CSS from {len(html_files)} HTML files (Removed ~44,000 repetitive lines)")
...
print("Stripped dummy attributes from models.")
...
for file in test_files:
    ...
    # Remove all test functions that have 'attribute_' or 'edge_case_' or 'error_handling_case_'
    # We will just split by 'def test_' and keep only the organic ones.
    lines = content.split('\n')
    clean_lines = []
    skip = False
    for line in lines:
...
print("Stripped repetitive loops from test files.")
